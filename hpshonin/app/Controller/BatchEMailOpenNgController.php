...
// 公開エラーメール クラス
class BatchEMailOpenNgController extends BatchAppController {
	...
	*/
	...
	public function execute() {
		$this->log(sprintf("公開エラーメールバッチを開始しました。(%d)", $this->id), LOG_INFO);

		$result = $this->execute_core($this->id);

		$this->log(sprintf("公開エラーメールバッチを終了しました。(%d)", $result), LOG_INFO);
		return $result;
	}
	...
	private function execute_core($package_id){
		...
		if(count($tos)){
			...
			$bodys
					...
					. "時間をおいて、再度、公開操作を行ってください。\n"
					. "\n"
					. "再操作を行ってもエラーとなる場合は、広報室まで\n"
					. "ご連絡ください。\n"
					. "\n"
