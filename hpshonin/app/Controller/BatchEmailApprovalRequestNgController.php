...
// 承認結果通知メール・承認完了通知メール(却下) クラス
class BatchEMailApprovalRequestNgController extends BatchAppController {
	...
	*/
	...
	public function execute() {
		$this->log(sprintf("承認結果通知メール・承認完了通知メール(却下) バッチを開始しました。(%d)", $this->id), LOG_INFO);

		//MTブログの作成
		$result = $this->execute_core($this->id);

		$this->log(sprintf("承認結果通知メール・承認完了通知メール(却下) を終了しました。(%d)", $result), LOG_INFO);
		return $result;
	}
