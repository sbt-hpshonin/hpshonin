...
(function ($) {
	$(function(){
		...
		$('#restore').click(function(){
			if(confirm("【注意】\n本番公開Webサーバーをこのパッケージの公開時点に戻します。\n処理を実施してよろしいですか？")){
				$('#package_edit_form').attr('action', '<?php echo $this->Html->url("/packages/restore/".$package['Package']['id']);?>' );
				$('#package_edit_form').submit();
			}
			return false;
		});
...
<div class="row-fluid">
			<div class="span12">
				<?php echo $this->Title->makeTitleBar("パッケージ詳細",h($project['Project']['project_name'])) ?>
				...
				<div class="block">
					...
					<tbody>
						...
						<tr>
							<td>コメント</td>
							<td><?php echo $this->Form->textarea('Package.camment', array('class' => 'span10','rows'=>10,'value'=>$package['Package']['camment'],'disabled'=>'disabled')); ?></td>
							</tr>
						<tr>
							<td>ステータス</td>
							<td>
								...
								if( $package['Package']['is_del'] == 1 ){
									...
									if($package['Package']['message'] != ""){
										echo "<br/><div class='text-error'>". nl2br( h($package['Package']['message'] )) . "</div>";
									}
								}
								?>
							</td>
					...
					<tbody>
						<tr>
							<td><?php // 承認依頼 ///////////////////////////////////////////////////////////////// ?>
								<?php // ボタンの場合 ?>
								<?php if ( $package['Package']['is_del'] == 0
										&& $package['Package']['status_cd'] == STATUS::STATUS_CD_PACKAGE_ENTRY
										&& ($roll_cd === AppConstants::ROLL_CD_ADMIN || $roll_cd === AppConstants::ROLL_CD_SITE) ) { ?>
								...
								<?php // 日付の場合 ?>
								 <?php } else if( $package['Package']['status_cd'] != STATUS::STATUS_CD_PACKAGE_READY
								 				&& $package['Package']['status_cd'] != STATUS::STATUS_CD_PACKAGE_READY_REJECT ){ ?>
							...
							<td><?php // 公開承認 ///////////////////////////////////////////////////////////////// ?>
								<?php // ボタンの場合 ?>
								<?php if ( $package['Package']['is_del'] == 0
										&& $package['Package']['status_cd'] == STATUS::STATUS_CD_APPROVAL_REQUEST ) { ?>
								...
								<?php } else if( $package['Package']['status_cd'] != STATUS::STATUS_CD_PACKAGE_READY
									...
									<?php } else if( $package['Package']['status_cd'] == STATUS::STATUS_CD_APPROVAL_OK
												|| $package['Package']['status_cd'] == STATUS::STATUS_CD_RELEASE_RESERVE
												|| $package['Package']['status_cd'] == STATUS::STATUS_CD_RELEASE_REJECT
												|| $package['Package']['status_cd'] == STATUS::STATUS_CD_RELEASE_READY ) { ?>
							...
							<td><?php // 公開設定 ///////////////////////////////////////////////////////////////// ?>
								...
								switch($package['Package']['status_cd']){
									...
									case Status::STATUS_CD_RELEASE_REJECT		:	/**  公開取消CD */
										break;
									case Status::STATUS_CD_RELEASE_READY		:	/** 公開事前準備 */
									...
									switch($package['Package']['status_cd']){
										...
										case Status::STATUS_CD_RELEASE_COMPLETE	:		/**  公開完了CD */
											print '<i class="icon icon-circle"></i>';
									 		if ($package['Package']['status_cd'] == Status::STATUS_CD_RELEASE_COMPLETE
												&& $package['Package']['id'] == $pre_package_id) {
												switch($roll_cd) {
													case AppConstants::ROLL_CD_ADMIN : // 管理者
													case AppConstants::ROLL_CD_DEVELOP : // 製作会社
													case AppConstants::ROLL_CD_SITE : // サイト担当者
														print '<br /><a href="javascript:void(0);" id="restore" class="btn">この時点に戻す</a>';
														break;
												}
											}
											break;
									...
									if($link_flg == false){
										...
										// ステータスが完了していないものは、ファイル一覧からリンクする
											...
											switch ($post["MtPost"]['modify_flg']){
												...
												case AppConstants::MODIFY_FLG_MOD: // 変更
												case AppConstants::MODIFY_FLG_NO_MOD: // 無変更
													echo '<a href="javascript:void(0);" onClick="window.open(\''. $this->Html->Url('diff_blog/'.$package['Package']['id'] ."/". $post["MtPost"]["edit_mt_post_id"]) .'\',\'diff\',\'width=1000,height=730,resizable=yes,menubar=no,toolbar=no,scrollbars=yes\'); return false;"><span class=\'file\'>'. $post["MtPost"]["subject"] .'</span></a>';
													echo '<a href="'. $post["MtPost"]['new_url'] .'" target="_blank"><i class="icon icon-globe"></i></a>';
													break;
												case AppConstants::MODIFY_FLG_DEL: // 削除
													echo "<span class='file1'>". $post["MtPost"]["subject"] ."</span>";
						...
						<tr data-tt-id='<?php echo $filedata['id'] ?>'  <?php if( $filedata['parent_id'] != "") { echo "data-tt-parent-id='".$filedata['parent_id']."'"; }?> >
							<td>
								...
								if( $filedata['mode'] == "folder") {
									...
									// (S)2013.11.07
									if($link_flg && $filedata['parent_id'] == "" && $package['Package']['is_blog']=="0"){
										if($package['Package']['operation_cd']== AppConstants::OPERATION_CD_PUBLIC){
											$base_url = $new_base_url;
										} else {
											$base_url = $old_base_url;
										}
										echo '<a href="'. $base_url . "/" . $project['Project']['site_url'] .'" target="_blank"><i class="icon icon-globe"></i></a>';
									}
									// (E)2013.11.07
								}else {
									// ファイルアイコン・記事名・地球儀アイコンの表示
										...
										if($package['Package']['operation_cd']== AppConstants::OPERATION_CD_PUBLIC){
											...
											switch ($filedata['modify_flg']){
												...
												case AppConstants::MODIFY_FLG_NO_MOD :
													echo '<a href="#" onClick="window.open(\''. $this->Html->Url('diff/'.$filedata['contents_files_id']) .'\',\'diff\',\'width=1000,height=730,resizable=yes,menubar=no,toolbar=no,scrollbars=yes\'); return false;"><span class=\''. ($filedata['mode']) . '\'>'. $filedata['text'] . '</span></a>';
													echo '<a href="'. $new_base_url . $filedata['filepass'] .'" target="_blank"><i class="icon icon-globe"></i></a>';
													break;
